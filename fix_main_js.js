...
const newOpen = `function openRecipeDetail(id){
    ...
    document.getElementById('rcp-steps-list').innerHTML = r.steps.map(function(st, idx){
      ...
      return '<div class="rcp-step-item">'
           ...
           + '<div class="rcp-step-title">B\\u01B0\\u1EDBc '+stNum+'</div>'
...
if (safeOldOpenRegex.test(mainJs)) {
    mainJs = mainJs.replace(safeOldOpenRegex, newOpen);
    fs.writeFileSync('frontend/src/main.js', mainJs, 'utf8');
    console.log('Successfully patched openRecipeDetail and fetchApi in main.js!');
} else {
    console.log('Regex fail in main.js');
}
